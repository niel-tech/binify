...
import { GET_BINS_BY_USER } from "../../../lib/fauna"
...
export default function Profile({ session_token }: ProfileProps) {
  const { data, error, isLoading } = useSWR(GET_BINS_BY_USER(session_token), { fallbackData: { getBinsByUser: [] } })
  const { getBinsByUser: bins = [] }: { getBinsByUser?: Bin[] } = data
      ...
      {bins.length > 0 &&
        bins.map((bin) => (
          ...
          >
            ...
            <div className="m-0 mb-1 w-full">
              <ValidUntil className="h-5 text-sm" validUntil={bin.lifetime} />
            </div>
          </div>
        ))}
      {bins.length === 0 && !isLoading && (
